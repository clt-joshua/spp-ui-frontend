...
- 8개 MVP 컴포넌트가 실제 앱 진입점에서 사용 가능하다.
...
- Button, IconButton, TextField, Checkbox, Select, Dialog, Menu, Snackbar
...
**결정:** 승인 프리셋은 빌드 산출물에 포함하고, 사용자 seed는 브라우저에서 MCU로 생성한다.

**이유:** 초기 렌더 안정성과 사용자 커스터마이징을 함께 제공한다.
...
### ADR-011 — Material Icons와 교체 가능한 Roboto를 기본 자산으로 사용

**결정:** Google Material Icons Filled와 Roboto Variable을 npm package로 self-host한다. 아이콘은 `MaterialIcon` adapter로, 서체는 `--app-font-brand`와 `--app-font-plain` reference input으로 격리한다.
...
### ADR-012 — GitHub Actions와 repository-owned Playwright baseline 사용

**결정:** `quality`, 3-browser `e2e`, Chromium/Linux `visual`을 GitHub Actions의 blocking gate로 사용한다. Playwright OS/browser image와 actionlint image는 digest까지 고정하고, visual baseline은 repository에 보관한다.

**이유:** 실제 Vite 진입점 검증, vendor-neutral 운영, 재현 가능한 pixel 비교를 함께 확보한다. browser별 동작 차이는 Chromium/Firefox/WebKit E2E로 확인하고 pixel baseline은 rendering 환경 차이를 피하기 위해 Chromium/Linux 한 환경만 authority로 사용한다.
